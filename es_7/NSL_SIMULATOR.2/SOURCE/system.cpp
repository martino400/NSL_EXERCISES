  ...
  if (input.is_open())
  {
    while (!input.eof())
    {
      ...
      if (property == "POTENTIAL_ENERGY")
      {
        ...
        index_property++;
        _vtail = 8.0 * M_PI * _rho * (1.0 / (9.0 * pow(_r_cut, 9.0)) - 1.0 / (3 * pow(_r_cut, 3.0))); // TO BE FIXED IN EXERCISE 7
      }
      ...
      else if (property == "PRESSURE")
      {
        ...
        index_property++;
        _ptail = 32.0 * M_PI * _rho * (1.0 / (9.0 * pow(_r_cut, 9.0)) - 1.0 / (6.0 * pow(_r_cut, 3.0))); // TO BE FIXED IN EXERCISE 7
      }
      else if (property == "GOFR")
      {
        for (int i = 0; i < _nblocks; i++)
        {
          ofstream coutgr("../OUTPUT/GOFR/gofr" + to_string(i + 1) + ".dat");
          coutgr << "# DISTANCE:  AVE_GOFR:     CUMUL_AVE_GOFR:   ERROR:" << endl;
          coutgr.close();
        }
        input >> _n_bins;
        _nprop += _n_bins;
...
// CHANGED THIS FOR EXERCISE 4.2
//  Read configuration from a .fcc file in directory ../OUTPUT/CONFIG/
// Qua ho detto di prendere in pbc halfside e quindi viene la metà in ovito
...
void System ::averages(int blk)
{
  ...
  _average = _block_av;
  _average /= _nsteps; // Normalize by number of steps per block
  _global_av += _average;
  ...
  if (_measure_gofr)
  {
    ...
    for (int i = 0; i < _n_bins; i++)
    {
      ...
      // _global_av2 stores sums of squares of block averages. When normalizing
      // we must divide by _gofr_norm^2 to get the square of the normalized
      // quantity. Previously the code divided by _gofr_norm which underestimates
      // the denominator inside the variance and yields an artificially large
      // error that grows with blocks.
...
// serve solo quando devo fare per tempi diversi
void System::set_temp_and_restart(double t)
{
  ...
  // Apertura in lettura con controllo
  ifstream in("../INPUT/input.dat");
  if (!in.is_open())
  {
    std::cerr << "Errore: impossibile leggere ../INPUT/input.dat (set_temp_and_restart)\n";
    return; // Esci subito per non cancellare il file
  }
  ...
  while (std::getline(in, line))
  {
    ...
    {
      ostringstream nl;
      // Mantengo una formattazione pulita
      nl << "TEMP                   " << t;
      lines.push_back(nl.str());
    }
    else
    {
      // Questo else ora cattura tutto ciò che NON è né TEMP né RESTART
  ...
  if (!out.is_open())
  {
    std::cerr << "Warning: impossibile scrivere in ../INPUT/input.dat (set_temp_and_restart)\n";
    return;
  }
