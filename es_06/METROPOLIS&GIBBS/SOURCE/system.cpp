...
{ // Propose a MC move for particle i
  ...
  { // Gibbs sampler for Ising
    ...
    prob = 1.0 / (1.0 + exp(-_beta * delta_E)); // Probability to have spin UP
    if (_rnd.Rannyu() < prob)
      // Se il numero che ho è minore della probabilità di averlo UP allora eseguilo
      _particle(i).setspin(1); // Gibbs acceptance step
    else
      _particle(i).setspin(-1); // Gibbs acceptance step
  ...
  while (!input.eof())
  {
    ...
    else if (property == "NPART")
    {
      ...
      for (int i = 0; i < _npart; i++)
      {
        ...
        // Se commento parte con tutti gli spin UP
...
void System ::initialize_velocities()
{
  ...
  { // TO BE TEMPORARILY CHANGED IN EXERCISE 4
    ...
    vec sumv(_ndim); // velocità del baricentro nelle varie dimensioni
    sumv.zeros();
  ...
  if (input.is_open())
  {
    while (!input.eof())
    {
      ...
      else if (property == "POFV")
      {
        ...
        input >> _n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0 * _temp / (double)_n_bins_v; // TO BE FIXED IN EXERCISE 4 --> PER
...
void System ::read_configuration()
{
  ...
  if (cinf.is_open())
  {
    ...
    cerr << "PROBLEM: Unable to open INPUT file config.ising" << endl;
  cinf.close();
...
{ // Measure properties
  ...
  {
    // va calcolato in H = 0
    //  A noi interessa il significato di fluttuazione dell'energia da uno stato del sistema a un altro stato del sistema
    //  Non interessa tra una particella e un'altra particella, interessa l'energia totale del reticolo
    _measurement(_index_cv) = _measurement(_index_tenergy) * _measurement(_index_tenergy); // salvo il singolo valore dell'energia
  }

  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////

  // siccome la media è 0 allora lo posso fare tutto in un momento solo
  if (_measure_magnet || _measure_chi)
  {
    double m = 0;
    double mag = 0.0;
    for (int i = 0; i < _npart; i++)
      mag += double(_particle(i).getspin());
    m = mag / double(_npart); // magnetization per spin
    if (_measure_magnet)
      _measurement(_index_magnet) = m;
    if (_measure_chi)
      _measurement(_index_chi) =  _npart * (m * m)*_beta; // store m^2; susceptibility will be computed in averages()
  }
...
void System ::averages(int blk)
{
  ...
  _average = _block_av / double(_nsteps); // in questo modo ho la magnetizzazione per particella che è quello che fa galli
  if (_measure_cv)
  {
    _average(_index_cv) = _beta * _beta * _npart * (_average(_index_cv) - _average(_index_tenergy) * _average(_index_tenergy)); // calore specifico per particella
  }
  _global_av += _average;
...
void System::set_temp_and_steps(double t)
{
  // Aggiorna direttamente lo stato del sistema
  _temp = t;
  _beta = 1.0 / _temp;
  _restart = true;
  ...
  if (!in.is_open()) {
    std::cerr << "Warning: impossible to read ../INPUT/input.dat\n";
    return;
  }
  ...
  while (std::getline(in, line))
  {
    ...
    if (key == "TEMP") 
    {
      ostringstream nl;
      nl << "TEMP                   " << t;
      lines.push_back(nl.str());
    }
    else if (key == "NSTEPS" && t < 0.8) 
    {
      // Aggiorna NSTEPS solo se la chiave corrisponde e t < 1.0
      ostringstream nl;
      nl << "NSTEPS                 " << 1000000;
      lines.push_back(nl.str());
    }
    ...
    {
      // Per tutte le altre righe (o per NSTEPS se t >= 1.0), mantieni l'originale
  ...
  std::ofstream out("../INPUT/input.dat", std::ios::trunc);
  if (!out.is_open())
  {
    std::cerr << "Warning: impossible to write ../INPUT/input.dat (set_temp_and_restart)\n";
    return;
  }
