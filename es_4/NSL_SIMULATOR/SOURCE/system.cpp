  ...
  if (input.is_open())
  {
    while (!input.eof())
    {
      ...
      else if (property == "POFV")
      {
        ...
        input >> _n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0 * _temp / (double)_n_bins_v; // TO BE FIXED IN EXERCISE 4 // Cosa già sensata da verificare che vada bene e non faccia una misura troppo grossolana
        _measure_pofv = true;
        _index_pofv = index_property;
        cout << "Index of POFV: " << _index_pofv << endl;
        cout << "Number of bins for POFV: " << _n_bins_v << endl;
...
void System ::averages(int blk)
{
  ...
  if (_measure_pofv)
  {
    ...
    for (int i = 0; i < _n_bins_v; i++)
    {
      ...
      coutf << setw(12) << i + 1
            << setw(12) << average/(double(_npart)*double(_nsteps))
            << setw(12) << sum_average / (double(blk) * double(_npart) * double(_nsteps))
            << setw(12) << this->error(sum_average/(double(_npart)*double(_nsteps)), sum_ave2/(double(_npart)*double(_nsteps)), blk) << endl;
    }
