...
//2-sphere norm between two cities
double TSP ::norm2_Sphere(int a, int b)
{
  // 2-sphere norm
  const double Earth_Radius = 6371.137;
  double theta_positions = 0.0; // this is the angle between the two cities
  theta_positions = acos(cities[a].x * cities[b].x + cities[a].y*cities[b].y+ cities[a].z*cities[b].z);
  double distance = Earth_Radius * theta_positions;
  return distance;
}
...
{
  ifstream in;
  string file_load = "../world_cities.txt";
  if(!in)
  {
    cerr << "Errore apertura 'world_cities.txt'" << endl;
  }
  in.open(file_load);
  int id = 0;
