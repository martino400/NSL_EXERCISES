...
void TSP ::create_citiesCIRC(string &info, Random &_rnd)
{
  ofstream out;
  ...
  {
    pair<double, double> coordinate = point_on_circle(_rnd);
    x = coordinate.first;
    y = coordinate.second;
...
// cities in a square
void TSP ::create_citiesSQUARE(string &info, Random &_rnd)
{
  ofstream out;
  // mi serve questa cosa per ordinare le città usando std library
  double x = 0.0, y = 0.0, theta = 0.0;
  ...
  for (int i = 0; i < n_cities; i++)
  {
    pair<double, double> coordinate = point_in_square(_rnd);
    x = coordinate.first;
    y = coordinate.second;
    cities.push_back({0, theta, x, y});
  }
...
// function to create a point on a circle
pair<double, double> TSP ::point_on_circle(Random &_rnd)
{
...
// function to create point inside a square
pair<double, double> TSP ::point_in_square(Random &_rnd)
{
  double x = 0.0, y = 0.0;
  // x and y coordinate
...
{
  ifstream in;
  string file_load = "../OUTPUT/CitiesInfo.dat";
  in.open(file_load);
  int id = 0;
  double val1 = 0.0, val2 = 0.0, val3 = 0.0;

  while (in >> id >> val1 >> val2 >> val3)
  {
