...
int main(int argc, char *argv[])
{
  ...
  for (int generation_count = 1; generation_count < GA.getGenerations(); generation_count++)
  {
    ...
    GA.reorder();
    int i1, i2;
    new_pop[0] = GA.getSpecTrajectory(0); // best trajectory of the previous generation
    ...
    double old_distance = GA.tot_dist(0);
    // calcola best della new_pop senza toccare population
    double new_best = 1e100;
    // find the shortest path
    for (int k = 0; k < GA.get_n_individuals(); ++k)
    {
      double d = GA.tot_dist_of_trajectory(new_pop[k]);
      // tira giù la distanza minore per new_pop in modo da confrontarla con quella della generazione prima
      if (d < new_best)
        new_best = d;
    }
    if (new_best <= old_distance)
    {
      // if evering ok save the new pop
    ...
    {
      // non accetto la nuova popolazione: riscrivo la vecchia popolazione sul file
      GA.paste_population(info); // opzionale: salva la popolazione attuale (vecchia)
    }
  }
  GA.find_loss_best();
  GA.find_loss_half();
